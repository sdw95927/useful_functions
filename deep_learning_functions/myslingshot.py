...
import sys
sys.path.append("/archive/DPDS/Xiao_lab/shared/hudanyun_sheng/projects/spatial_transcriptomics_pseudotime/pyslingshot")
from slingshot.util import scale_to_range, mahalanobis
from slingshot.lineage import Lineage
from slingshot.plotter import SlingshotPlotter


def find_parent(child, tree_str):
    for (key, item) in tree_str.items():
        if child in item:
            return key
    return "root"

def select_starting_node(K, slingshot):
    trees = []
    len_trees =[]
    for k in range(K):
        tree_k = slingshot.construct_mst(k) # the k-th tree

        leaves = [k for k,v in tree_k.items() if v ==[]] # childless leaves
        num = len(leaves)
        print(f"num leaves: {num}")

        lens = []
        tree_k_ = []
        for p in leaves: # for every childless leave
            path_p = []
            lenp = 0
            while p is not "root":
                lenp += 1
                p = find_parent(p, tree_k)
                path_p.append(p)
            tree_k_.append(path_p)
            lens.append(lenp)
    #     len_tree = lens.sum()   
        trees.append(tree_k_)
        len_trees.append(sum(lens)/num)
    return np.argmax(np.array(len_trees)), len_trees


class Slingshot():
    def __init__(self, data, cluster_labels, start_node=None, end_nodes=None, debug_level=None):
        self.data = data
...
#         for start_node in np.unique(self.cluster_labels):
#             import pdb; pdb.set_trace()
#             self.construct_mst(start_node)
    ...
    def fit(self, num_epochs=10, debug_axes=None, tol=1e-3):
        ...
        print(f"initial distances: {[x.sum() for x in self.distances]}")
        d_sq_old = [np.Inf for x in self.distances]
...
#             d_sq = [x.sum() for x in self.distances]
            d_sq = [x.mean() for x in self.distances]
            diffs = [np.abs(x - y) for (x,y) in zip(d_sq, d_sq_old)]
            print(f"end of epoch {epoch}, distances: {[x.sum() for x in self.distances]}, change of distances: {diffs}")
            
            if (epoch == num_epochs - 1) or (np.array(diffs)<tol).all():  # plot curves
                self.plotter.clusters(self.debug_axes[1, 1], s=2, alpha=0.5)
                self.plotter.curves(self.debug_axes[1, 1], self.curves)
            
            if (np.array(diffs)<tol).all():
                break
            d_sq_old = d_sq
    ...
    def fit_lineage_curves(self):
        ...
        for l_idx, lineage in enumerate(self.lineages):
            ...
            d_sq, dist = curve.project_to_curve(self.data, curve.points_interp[curve.order]) # dist = d_sq.sum()
            distances.append(d_sq)
        self.distances = distances
        if self.debug_plot_lineages:
